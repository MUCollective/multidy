...
# AMK: this needs to remain a branch because r values should not be adjusted in analysis paths where we use approach as a covariate
...
Now, we are ready to model our data. Multiple decisions impact the implementation here, so we use conditions to constrain our modeling decision based on *decisions we have already made*.

```{multiverse, modeling, inside = M}
m <- branch(model,
  # linear models
  "lm" %when% (operationalize_jnd != "hierarchical" && handle_non_normality == "exclude" && adjust_for_approach_bias == "adjusted_correlation") ~
    lm(jnd ~ r * visandsign, data = df),
  "lm_approach" %when% (operationalize_jnd != "hierarchical" && handle_non_normality == "exclude" && adjust_for_approach_bias == "approach_as_covariate") ~
    lm(jnd ~ r * visandsign * approach_value, data = df),
  # "lmer" %when% (operationalize_jnd != "hierarchical" && handle_non_normality == "exclude" && adjust_for_approach_bias == "adjusted_correlation") ~ 
  #   lmer(jnd ~ r * visandsign + (1 | participant), data = df),
  
  # log-linear models
  "glm" %when% (operationalize_jnd != "hierarchical" && handle_non_normality == "log_transform" && adjust_for_approach_bias == "adjusted_correlation") ~
    glm(jnd ~ r * visandsign, data = df, family = gaussian(link = "log")),
  "glm_approach" %when% (operationalize_jnd != "hierarchical" && handle_non_normality == "log_transform" && adjust_for_approach_bias == "approach_as_covariate") ~
    glm(jnd ~ r * visandsign * approach_value, data = df, family = gaussian(link = "log"))#,
  # "glmer" %when% (operationalize_jnd != "hierarchical" && handle_non_normality == "log_transform" && adjust_for_approach_bias == "adjusted_correlation") ~
  #   glmer(jnd ~ r * visandsign + (1 | participant), data = df, family = gaussian(link = "log"))#,
  
  # TODO: add hierarchical binomial paths
  # # hierarchical logistic regression and derive JNDs after
  # "glmer_binomial" %when% (operationalize_jnd == "hierarchical" && adjust_for_approach_bias == "adjusted_correlation") ~
  #   glmer(correct ~ r * log(dr) * visandsign + (1 | participant), data = df, family = binomial),
  # "glmer_binomial_approach" %when% (operationalize_jnd == "hierarchical" && adjust_for_approach_bias == "approach_as_covariate") ~
  #   glmer(correct ~ r * log(dr) * visandsign * approach_value + (1 | participant), data = df, family = binomial)
)
```

Now we post-process predictions from our models. The way we do this depends on how we modeled our data. Again, we uses conditions to represent links between universes.

```{multiverse, postprocessing, inside = M}
postprocess <- branch(extract_estimates,
  "extract_jnds_lm" %when% (model == "lm") ~ function(m) {
  ...
  "extract_jnds_lm_approach" %when% (model == "lm_approach") ~ function(m) {
    # extract marginal estimate for JNDs in each level of visandsign
    output <- m %>% 
      emmeans(~ visandsign, 
        cov.keep = c("r"), 
        at = list(approach_value = c(0)), # this line is different from above
        type = "response"
      ) %>% 
      summary() %>% # to dataframe
      rename(
  ...
  # "extract_jnds_lmer" %when% (model == "lmer") ~ function(m) {
  #   # extract marginal estimate for JNDs in each level of visandsign
  #   output <- m %>% 
  #     emmeans(~ visandsign, 
  #       cov.keep = c("r"), 
  ...
  #   return(output)
  # },
  "extract_jnds_glm" %when% (model == "glm" || model == "glmer") ~ function(m) {
    # extract marginal estimate for JNDs in each level of visandsign
    output <- m %>% 
      emmeans(~ visandsign, 
        cov.keep = c("r"), 
      ...
      rename(
        jnd = response,
        lowerCI = asymp.LCL, # these lines are different from above
        upperCI = asymp.UCL
      )

    return(output)
  },
  "extract_jnds_glm_approach" %when% (model == "glm_approach") ~ function(m) {
    # extract marginal estimate for JNDs in each level of visandsign
    output <- m %>% 
      emmeans(~ visandsign, 
        cov.keep = c("r"), 
  ...
  # TODO: revise postprocessing for hierarchical binomial path to use emmeans()
...
We can unpack the multiverse by using `expand`.

```{r}
M_expand <- multiverse::expand(M)
```

This enables us to view generated code for each universe.
...
Now lets pull our output dataframes out of the multiverse.
...
We'll process these results to create a p-box representation of uncertainty that encodes the min and max of the 95% confidence intervals for JND estimates at each level of `visandsign` across universes.
...
Now let's make a chart summarizing this analysis. Each black dot is the average JND estimate at each level of `visandsign` across universes. Each confidence interval is a p-box aggregating the sampling distributions of JND estimates across universes. Each gray dot is a point estimate from an individual universe.

```{r}
vis_df %>% ggplot(aes(x = visandsign)) +
  geom_errorbar(aes(ymin = min_jnd, ymax = max_jnd), width=.1) +
  geom_point(aes(y = avg_jnd), position = position_identity()) +
  geom_point(aes(y = jnd), alpha = 0.3, position = position_jitter()) +
  theme_minimal()
```
