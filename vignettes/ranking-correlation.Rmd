...
df <- branch(handle_performance_at_chance,
        ...
        "censor" ~ df %>% 
          mutate(
            ...
            jnd = Surv(pmin(jnd, censoring_threshold), !censored) # censored JNDs
          )
)
```
...
```{r}
results <- extract_variables(M, m)
```

```{r}
m <- gamlss(jnd ~ r * visandsign * NULL + random(participant), 
        sigma.formula = ~visandsign, data = df, family = LOGNO)
```
