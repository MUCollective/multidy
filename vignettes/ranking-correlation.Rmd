...
param_from_curve <- function(fit, param) {
...
df <- branch(operationalize_jnd,
  ...
  "curve_fitting" ~ df %>%
    ...
    mutate(
      fit = map2(dr, correct, ~gamlss(.y ~ log(.x), family = BI)), # logistic regression with log stimulus intensity as predictor
      # b_0 = map(fit, ~coef(.x))[[1]],
      # b_1 = map(fit, ~coef(.x))[[2]],
      b_0 = param_from_curve(fit, "intercept"),
      b_1 = param_from_curve(fit, "slope"),
      jnd = exp(jnd_from_logistic(b_0, b_1))
    ) %>%
    unnest(cols = c(dr, correct)) %>%
    ungroup(),
  "hierarchical" ~ df # derive jnds after model fitting
)
```
...
Here, we create and apply a logical index for exclusion criteria. This includes two decision points: 
 ...
 2. Do we handle non-normality by excluding outliers (as in Harrison et al.) or by log-transforming our JNDs later on (as in Kay and Heer)?
...
df <- df %>%
  ...
  mutate(
    p_chance_cutoff = branch(handle_performance_at_chance,
      "exclude" ~ mean(jnd > .45) > 0.2, # exclude visandsign with > 20% jnds worse than chance performance (> 0.45)
...
m <- branch(random_effects,
  "yes" %when% {operationalize_jnd != "hierarchical"} ~ 
    gamlss(
      ...
      data = df,
      family = branch(handle_performance_at_chance,
        ...
        "censor" ~ # censored models
          cens(LOGNO) # only log-linear censored models converge well, consistent with Kay and Heer's choice not to use cens(NO)
          # branch(handle_non_normality,
          #   "exclude" ~ cens(NO), # linear models
          #   "log-transform" ~ cens(LOGNO) # log-linear models
          # )
      )
    ),
...
postprocess <- branch(operationalize_jnd,
    ...
    "curve_fitting" ~ function(m, df) { # extract predicted JNDs from model
        output <- expand.grid(r = unique(df$r), visandsign = levels(factor(df$visandsign)), participant = levels(factor(df$participant))) %>%
          branch(adjust_for_approach_bias,
            "adjusted-correlation" ~ (function(d) { return(d) }), # do nothing
            "approach-as-covariate" ~ mutate(approach_value = 0)
