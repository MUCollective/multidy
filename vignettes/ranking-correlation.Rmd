...
<!-- We are using `gamlss` for modeling, and this package makes an assumption that the data the model is fit to will be found in the global evironment scope. This assumption is fine for normal use cases, but it does not work with `multiverse` which creates a separate environment for each universe. Without a workaround, `gamlss` will fit to the wrong data, and there will be a mismatch between our preprocessing and the model spec. We define some wrapper functions that deal with this scope issue as a work-around. -->

<!-- ```{r} -->
<!-- gamlss_ = function(..., data = sys.parent()) { -->
<!--   dd = data -->
<!--   m = gamlss(..., data = dd) -->
<!--   m$data = data -->
<!--   m -->
...
<!--   old_dd = get0("dd", envir = globalenv(), inherits = FALSE) -->
<!--   assign("dd", object$data, envir = globalenv()) -->
<!--   on.exit( -->
<!--     if (is.null(old_dd)) rm(dd, envir = globalenv()) -->
<!--     else assign("dd", old_dd, envir = globalenv()) -->
<!--   ) -->
<!--   predict(object, ...) -->
<!-- } -->
<!-- ``` -->
...
m <- branch(random_effects,
  "yes" %when% (operationalize_jnd != "hierarchical") ~
      ...
      "log_transform" ~ # log-linear model
        glmer(jnd ~ r * visandsign
            * branch(adjust_for_approach_bias, # conditions avoid fit issues in specific universes
                "adjusted_correlation" ~ NULL, #%when% (operationalize_jnd != "staircase_convergence")
                "approach_as_covariate" ~ approach_value #%when% (operationalize_jnd == "guess_lapse_curve_fitting")
              )
            + (1 | participant),
          data = df,
    ...
    #     "log_transform" ~ LOGNO # log-linear models
    #   )
    # ),
...
postprocess <- branch(operationalize_jnd,
    "staircase_convergence" ~ function(m) {
      ...
      # # set up data grid for prediction
      # data_grid <- expand.grid(r = unique(df$r), visandsign = levels(factor(df$visandsign)), participant = levels(factor(df$participant))) %>%
      #   branch(adjust_for_approach_bias,
      #     "adjusted_correlation" ~ (function(d) { return(d) }), # do nothing
      #     "approach_as_covariate" ~ mutate(approach_value = 0)
      #   )
      # 
      # # extract predicted JNDs from model
      # prediction <- predict(m, se.fit = TRUE, newdata = data_grid, data = df, type = "response")
      # 
      # output <- data_grid %>%
      #   mutate(
      #     # unpack fitted predictions and standard errors to dataframe
      #     fit = prediction$fit,
      #     se.fit = prediction$se.fit,
      #     # simulate draws from a t distribution to transform into a sampling distribution of JNDs
      #     .draw = list(1:5000),
      #     t = map(df, ~rt(5000, .))
      #   ) %>%
      #   unnest(cols = c(".draw", "t")) %>%
      #   # scale and shift t to get a sampling distribution of average JNDs for each condition in the data grid
      #   mutate(jnd = t * se.fit + fit) %>%
