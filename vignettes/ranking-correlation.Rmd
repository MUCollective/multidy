...
Using approach as a covariate in models with random effects leads to identifiability issues, so we omit these paths
...
postprocess <- branch(extract_estimates,
  ...
  "extract_jnds_glm.er" %when% (model == "glm" || model == "glmer") ~ function(m) {
    # extract marginal estimate for JNDs in each level of visandsign
    output <- m %>% 
      emmeans(~ visandsign, 
        cov.keep = c("r"), 
